...
class Propiedades extends REST_Controller
{
  ...
  function exportar_inmobusquedas()
  {
    ...
    if (sizeof($propiedades["results"]) > 0) {
      ...
      foreach ($propiedades["results"] as $p) {
        ...
        $p->texto = preg_replace('/[\x00-\x1F\x7F\xA0]/u', '', $p->texto);
  ...
  function get_propiedades()
  {
    ...
    ini_set('display_startup_errors', 1);
    error_reporting(E_ALL);
    $id_empresa = isset($_POST["id_empresa"]) ? $_POST["id_empresa"] : parent::get_empresa();
    $id_localidad = isset($_POST['id_localidad']) ? $_POST["id_localidad"] : 0;
    $tipo_operacion = isset($_POST["tipo_operacion"]) ? $_POST["tipo_operacion"] : 0;
    $tipo_propiedad = isset($_POST["tipo_propiedad"]) ? $_POST["tipo_propiedad"] : 0;
    $dormitorios = isset($_POST["dormitorios"]) ? $_POST["dormitorios"] : 0;
    $limit = isset($_POST["limit"]) ? $_POST["limit"] : 0;
    $offset = isset($_POST["offset"]) ? $_POST["offset"] : 0;
    $rest = $this->modelo->buscar(
      array(
        'id_empresa' => $id_empresa,
        'id_localidad' => $id_localidad,
    ...
    /* print_r($rest); */
    $propiedades = [];
    foreach ($rest["results"] as $html) {
      $prop = "<div class='col-md-4 p-0 neighborhoods-list'>";
      $prop .= "<a href='" .  $this->mklink($html->link) . "'>";
      $prop .= "<div class='img-block'>";
      $prop .= "<img src=" . '/admin/' . $html->path  . "  alt='img'>";
      $prop .= "<div class='neighborhoods-top'>";
      if (!empty($html->calle)) {
        $prop .= "<p>" . $html->calle . ($html->localidad != '' ? "," . $html->localidad : '') . " </p>";
      }
      if ($html->publica_precio == 1) {
        if ($html->precio_final != 0) {
          $prop .= " <h4>" . $html->moneda . $html->precio_final . "</h4>";
        }
      } else {
        $prop .= "<h4>Consultar</h4>";
      }
      $prop .= "</div>";
      $prop .= "<div class='neighborhoods-bottom'>";
      if ($html->ambientes != 0) {
        $prop .= "<div class='neighborhoods-info'>";
        $prop .= "<h6>" . $html->ambientes . " Hab. </h6>";
        $prop .= "<img src='assets/images/icon11.png' alt='img'>";
        $prop .= "</div>";
      }
      if ($html->banios != 0) {
        $prop .= "<div class='neighborhoods-info'>";
        $prop .= "<h6>" . $html->banios . " Baños </h6>";
        $prop .= "<img src='assets/images/icon12.png' alt='img'>";
        $prop .= "</div>";
      }
      if ($html->cocheras != 0) {
        $prop .= "<div class='neighborhoods-info'>";
        $prop .= "<h6>" . $html->cocheras . "</h6>";
        $prop .= "<img src='assets/images/icon13.png' alt='img'>";
        $prop .= "</div>";
      }
      if ($html->superficie_total != 0) {
        $prop .= "<div class='neighborhoods-info'>";
        $prop .= "<h6>" . $html->superficie_total . "</h6>";
        $prop .= "<img src='assets/images/icon14.png' alt='img'>";
        $prop .= "</div>";
      }
      $prop .= "</div> </a> </div>  </div>";
      $propiedades[] = $prop;
