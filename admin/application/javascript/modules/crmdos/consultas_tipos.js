(function ( models ) {
  ...
  models.ConsultatipoDos = Backbone.Model.extend({
...
(function (collections, model, paginator) {
  collections.ConsultastiposDos = paginator.requestPager.extend({
    model: model,
    paginator_core: {
      url: "consultas_tipos/"
    }
  });
})( app.collections, app.models.ConsultatipoDos, Backbone.Paginator);


(function ( app ) {

  app.views.ConsultastiposTableViewDos = app.mixins.View.extend({

    template: _.template($("#consultas_tipos_tree_panel_template_dos").html()),
    ...
    myEvents: {
      "click .editar":function(e) {
        ...
        var cat = new app.models.ConsultatipoDos({ id: id });
        cat.fetch({
          "success":function(){
            self.ver(cat);
          }
    ...
    ver: function(modelo) {
      var categoria = new app.views.ConsultatipoEditViewDos({
        model: modelo,
        permiso: 3,
      });
...
(function ( views, models ) {
  ...
  views.ConsultatipoEditViewDos = app.mixins.View.extend({

    template: _.template($("#consultas_tipos_edit_panel_template_dos").html()),
    ...
    eliminar : function() {
      ...
      var self = this;      
      var consulta_tipo = new app.models.ConsultatipoDos({
        "id":self.model.id
      });
    ...
    limpiar : function() {
      this.model = new app.models.ConsultatipoDos();
      this.render();
    },
