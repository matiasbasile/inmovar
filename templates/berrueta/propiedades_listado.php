...
function filter() {
  global $vc_offset, $vc_view, $vc_total_paginas, $vc_link, $vc_page, $vc_tipo_operacion, $vc_order; ?>
  <div class="filter">
    <ul class="tab-button">
    ...
    <div class="sort-by">
      ...
      <select id="sort-by" onchange="filtrar2()">
        <option <?php echo($vc_order == 0)?"selected":"" ?> value="0">Recientes</option>
        <option <?php echo($vc_order == 1)?"selected":"" ?> value="1">Precio Mayor a Menor</option>
        <option <?php echo($vc_order == 2)?"selected":"" ?> value="2">Precio Menor a Mayor</option>
      </select>
    </div>
    <?php if ($vc_total_paginas > 1) { ?>
      <div class="pagination">
        <?php if ($vc_page > 0) { ?>
            <a class="prev" href="<?php echo $vc_link.($vc_page-1); ?>/"></a>
        <?php } ?>
        <?php for($i=0;$i<$vc_total_paginas;$i++) { ?>
            <?php if (abs($vc_page-$i)<2) { ?>
              <a class="<?php echo ($i==$vc_page) ? "active" : ""?>" href="<?php echo $vc_link.$i; ?>/"><?php echo ($i+1); ?></a>
            <?php } ?>
        <?php } ?>
        <?php if ($vc_page < ($vc_total_paginas-1)) { ?>
            <a class="next" href="<?php echo $vc_link.($vc_page+1); ?>/"></a>
        <?php } ?>
      </div>
    <?php } ?>
    <div class="present">
    ...
    <div class="page">
      <div class="breadcrumb">
        ...
        <?php if (!empty($vc_tipo_operacion)) { ?>
          <a href="<?php echo mklink("propiedades/".($vc_tipo_operacion->nombre)) ?>"></a>
      ...
      <big><?php echo (!empty($vc_tipo_operacion)) ? ($vc_tipo_operacion->nombre) : "Propiedades"; ?></big>
    </div>
  </div>
</div>
