...
class Propiedad_Model {
  ...
  function get_variables($config = array()) {
    ...
    $orden_defecto = (isset($config["orden_default"]) ? $config["orden_default"] : 4);
  ...
  function get_by_tokko_id($tokko_id = "") {
    $sql = "SELECT id, id_empresa FROM inm_propiedades WHERE tokko_id = '$tokko_id' ";
    $q = mysqli_query($this->conx,$sql);
    if (mysqli_num_rows($q)>0) {
      $r = mysqli_fetch_object($q);
      return $this->get($r->id,array(
  ...
  private function encoding($p) {
    ...
    if ($p->id_empresa == 1575 && $p->id_tipo_estado == 7 && $p->id_tipo_operacion != 2) $p->tipo_estado = "Nuevo Proyecto";
  ...
  function get_list($config = array()) {
    ...
    if (!empty($order_by)) {
      ...
      if ($order == 1) {
        ...
        $sql.= "ORDER BY IF(A.id_empresa = $this->id_empresa, IF(A.destacado = 1, 0, 1) ,2) ASC, A.precio_final ASC, A.id DESC ";
      } else if ($order == 5) {
